 ...
 * ── 设计约束（改的时候别破坏）─────────────────────────────
 *
 * 1. 金色预算约束的是**界面**,不是内容。按钮、边框、标签、筛选态里,
 *    金色整屏不超过三处。卡面图是链上现算的证书,它自带金框与金色纹样
 *    ——那是合约的输出、是这个产品的实物,不为 UI 规则改写(也改不了,
 *    合约已部署)。把它算进 UI 预算,等于让规则吃掉产品本身。
 *
 * 2. 稀有度在界面里是文字不是色块(卡面 SVG 里的徽章属于内容,同上)。
 *    黑金卡的辨识不再依赖"唯一有金边"——每张卡面都有金框,那条特权已经
 *    名存实亡——改用外发光,这是界面层能给的、其他卡没有的信号。
 *
  ...
  return (
    ...
    >
      ...
      */}
      ...
      {/*
        文字区只有一行,而且不重复卡面上已有的东西。
        卡面印的是英文规范名 + 稀有度 + 价格 + 铸造量 + 能耗,所以:
        · 中文界面:这里放中文名(卡面恒为英文,中文用户需要它)
        · 英文界面:名字已经在卡面上了,这里改放创作者 —— 卡面没有创作者,
          而它是"谁做的这张卡"这个问题的答案,比再念一遍名字有用。
        没有卡面图时(回退版式)一律显示名字,那时卡上没有任何文字。
      */}
      <div className="flex items-baseline justify-between gap-2 px-3.5 py-3">
        ...
        >
          {artUri && text.title === skill.title ? skill.creator : text.title}
        </h3>
